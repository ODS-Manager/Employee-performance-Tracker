...
import { formatStoredDate, getPacificWeekStartDate, isUserRole } from '../../utils/helpers'
...
export const TeamLeadTargetsPage = () => {
  ...
  useEffect(() => {
    if (!user || !isUserRole(user.userRole, 'admin') && !isUserRole(user.userRole, 'superadmin')) {
      navigate('/login')
    }
  }, [user, navigate])
  ...
  const { data: targetsData, isLoading: loadingTargets } = useQuery({
    ...
    enabled: !!user && (isUserRole(user.userRole, 'admin') || isUserRole(user.userRole, 'superadmin')),
  })
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <AdminHeader title="Team Lead Targets" subtitle="Set weekly productivity targets for team leads" />
      <AdminNav />

      <main className="container mx-auto px-4 py-8">
        <Tabs defaultValue="team-leads" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 max-w-md">
            <TabsTrigger value="examiners" onClick={() => navigate('/admin/examiner-targets')}>
              Examiners
            </TabsTrigger>
            <TabsTrigger value="team-leads">
              Team Leads
            </TabsTrigger>
          </TabsList>

          <TabsContent value="team-leads" className="space-y-6">
            {/* Week Navigation */}
            <Card>
...
export default TeamLeadTargetsPage
